...
/*
  Simple Serial
  Landuage: Arduino/Wiring
  Listens for an incoming serial byte, addes one to the byte
    and sends the result back over serially.
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    inByte = Serial.read();
    // add one to it, then send the result out:
